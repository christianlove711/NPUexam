"""第二版确定性候选生成器：拓扑排序 → 连续分块 → 分核排程 → 近似排序。

这里的时间和内存分数只用于筛选候选，最终合法性和 Makespan 以官方评估为准。
复用第一版的操作图构造与回退方案；复制项目时必须保留第一版 code 目录。
具体近似公式、输入输出见《算法与实验方案.md》和《代码结构与函数说明.md》。
"""
...
ATTACHMENT = PROJECT.parent
OFFICIAL_CODE = ATTACHMENT / "code"
LEGACY_CODE = ATTACHMENT / "多核调度_第一版" / "code"
if not (OFFICIAL_CODE / "evaluation_validation.py").is_file():
    raise RuntimeError(f"Official evaluator is missing: {OFFICIAL_CODE}")
if not (LEGACY_CODE / "solve_multicore_v1.py").is_file():
    raise RuntimeError(f"First version is missing: {LEGACY_CODE}")
os.environ["MULTICORE_OFFICIAL_CODE"] = str(OFFICIAL_CODE)
for directory in (str(OFFICIAL_CODE), str(LEGACY_CODE)):
...
from solve_multicore_v1 import _graph_view, generate_plan as generate_v1  # noqa: E402
...
def generate_candidates(graph, num_cores, scene, config, limit=2):
    """生成并校验候选，按近似评分返回至多 limit 个 (标签, 方案, 分数)。

三种拓扑序乘至多四档目标规模，默认仅保留两份给 worker 正式评估。
这里不评估第一版回退方案；由 worker 统一比较官方成绩。
    """
...
def legacy_plan(graph, num_cores, scene):
    """调用保留的第一版生成器，提供同场景、同核数的比较与回退方案。"""
    return generate_v1(graph, num_cores, scene)
