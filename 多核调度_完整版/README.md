...
本目录包含第二版求解器、题目一至三的批量评估、可复现结果和交接材料。赛题原有 `code/`、`data/`、`docs/` 和 `多核调度_第一版/` 位于本目录的上一级；运行时不要改动官方评估代码或 `data/config.txt`。把整个赛题附件目录复制到另一台电脑即可保留这些相对路径。

当前分工为 **Mac 编写算法，强 CPU 电脑运行评估和搜索**。以下实验命令在目标运行电脑执行；无需为了阅读文档在 Mac 上重跑。
...
```bash
python 多核调度_完整版/run_all.py --cases case_019 --cores 2 --search-hours 0
```
...
```bash
python code/multicore_cut_evaluate_problem_1.py data/case_019.json "多核调度_完整版/runs/<运行编号>/problem_1/plans/case_019_2core.json" --config data/config.txt -o "多核调度_完整版/runs/<运行编号>/problem_1/case_019_2core_full.json" --trace-output "多核调度_完整版/runs/<运行编号>/problem_1/case_019_2core_trace.json" --log-output "多核调度_完整版/runs/<运行编号>/problem_1/case_019_2core_log.txt"
```
