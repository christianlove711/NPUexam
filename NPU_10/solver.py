...
def refinement_sizes(coarse_size):
    """The three fourth-version neighbor grains around a coarse winner."""
    return {4: (1, 2, 8), 16: (8, 24, 32), 48: (24, 32, 96),
            192: (96, 128, 384), 768: (384, 512, 1536)}.get(int(coarse_size), ())
...
def legal_insertions(view, subgraph, target_row):
    """Positions respecting all direct quotient-DAG edges on the target core."""
    position = {sg: index for index, sg in enumerate(target_row)}
    before = max((position[sg] + 1 for sg in view["subgraph_preds"][subgraph]
                  if sg in position), default=0)
    after = min((position[sg] for sg in view["subgraph_succs"][subgraph]
                 if sg in position), default=len(target_row))
    if before > after:
        return ()
    return tuple(dict.fromkeys((before, (before + after) // 2, after)))


def reaches(view, source, target):
    """Test transitive subgraph dependence before a same-core swap."""
    ...
    while pending:
        ...
        if current not in seen:
            seen.add(current)
            pending.extend(view["subgraph_succs"][current] - seen)
    return False
...
def neighbors(graph, plan, scene, limit=32):
    ...
    for sg in movable:
        ...
        for target in targets:
            for position in legal_insertions(view, sg, schedules[target]):
                rows = copy.deepcopy(schedules)
    ...
    for core, row in enumerate(schedules):
        for index in range(len(row) - 1):
            if reaches(view, row[index], row[index + 1]):
                continue
            rows = copy.deepcopy(schedules)
