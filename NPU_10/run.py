...
from polish import local_candidates, depth_candidates, sibling_candidates
from itertools import islice, zip_longest
from inherit import checked_source, transfer
...
def worker(run, case, cores):
    ...
    for scene in scenes:
        frozen = scene not in opts.get('search_scenes',list(FOLDERS))
        requested_budget = 0 if frozen else opts['large_budget'] if large else opts['small_budget']
        ...
        seen = set()
        failed_hashes = {e['plan_hash'] for e in data.get('inherited_errors',[]) + errors
                         if e.get('plan_hash') and 'timeout' in e.get('error','')}
        preflight_rejections = 0
        cache_dirty = False

        def checkpoint(done=False, reason='searching'):
            nonlocal best,cache_dirty
            data['best'] = best
            ...
            if cache_dirty or not archive_path.exists():
            ...
            row['preflight_rejections'] = preflight_rejections
            atomic_json(pp,best['plan'])
            atomic_json(jp,row)

        def consider(label, plan, required=False):
            nonlocal calls,valid,best,hits,pair_attempts,cache_dirty
            ...
            if record:
                ...
                if digest in failed_hashes and not required:
                    return None
                ...
                if not result['ok']:
                    ...
                    return None
                valid += 1
                metric, origin = clean(result['metrics']), 'NPU_10 official evaluation'
        ...
        if budget and (opts.get('polish_strong') or not strong) and calls < budget + pair_attempts and time.monotonic()-started < wall:
            ...
            def proposals():
                ...
                # Spend new calls on the incumbent's neighborhood, not another
                # pass through the same exhausted structural portfolio.
                local = local_candidates(graph,best['plan'],scene,config,best['metrics'])
                depths = depth_candidates(graph,cores,scene,config,best['label'])
                siblings = sibling_candidates(graph,cores,scene,config)
                for pair_of_candidates in zip_longest(islice(siblings,3),islice(local,8),islice(depths,4)):
                    for item in pair_of_candidates:
                        if item is not None:
                            yield item
                for label,plan in candidates(graph,cores,scene,config):
                    if label=='depth_islands_2' and len(graph['ops'])>5000:
                        continue
                    yield label,plan
                for label,p in partition_variants(graph,best['plan'],scene,config):
                    yield 'best_'+label,p
                yield from local_candidates(graph,best['plan'],scene,config,best['metrics'])
            try:
                ...
                while calls < budget + pair_attempts and time.monotonic()-started < wall:
                    try:
                        ...
                        break
                    digest = canonical_hash(plan)
                    if digest in seen or digest in failed_hashes:
                        continue
                    if digest in data['cache']:
                        consider(label,plan)
                        continue
                    ...
                    if max(loads.values(),default=0) > best['metrics']['makespan']:
                        continue
                    try:
                        from official import validate_candidate_structure
                        validate_candidate_structure(graph,plan,scene,
                            config if scene!='A' and label.startswith('local_') else None)
        ...
        checkpoint(done, 'inherited_frozen' if frozen else 'pair_pending' if not done else
...
def clean_cli_path(value):
    r"""Accept paths copied from Markdown where underscores became ``\_``."""
    if value is None:
        return None
    return Path(str(value).replace('\\_', '_'))


def main():
    p = argparse.ArgumentParser(description=__doc__)
    p.add_argument('--run',type=Path)
    p.add_argument('--upstream-run',type=Path,nargs='+',default=[],help='Completed A/B runs to inherit on creation')
    ...
    args.run = clean_cli_path(args.run)
    args.upstream_run = [clean_cli_path(path) for path in args.upstream_run]
    if args.upstream_run and args.run and (args.run/'manifest.json').exists():
        p.error('--upstream-run is only for creating a new run')
    ...
    if args.run and (args.run/'manifest.json').exists():
        ...
        if args.refine:
            ...
            for key in manifest['options']:
                if key != 'search_scenes':
                    manifest['options'][key] = getattr(args,key)
            manifest['options']['search_scenes'] = args.scenes
        ...
        upstream = [(s.resolve(),checked_source(s)) for s in args.upstream_run]
        for source,m in upstream:
            if not set(cases)<=set(m['cases']) or not set(args.cores)<=set(m['cores']):
                raise ValueError(f'Upstream does not cover requested cases/cores: {source}')
        manifest = {'algorithm':'NPU_10','cases':cases,'cores':list(dict.fromkeys(args.cores)),
        ...
        manifest['options']['search_scenes'] = args.scenes
        manifest['upstream_runs'] = [str(s) for s,m in upstream]
        for case in cases:
            if not upstream:
                break
            for cores in manifest['cores']:
                name = f'{case}_{cores}.json.gz'
                pack = load_pack(ROOT/'seedpack'/name)
                for source,m in upstream:
                    if score(read_json(source/'baseline/jobs'/f'{case}.json')) != score(read_json(ROOT/'seedpack/baseline/jobs'/f'{case}.json')):
                        raise ValueError('Upstream baseline mismatch')
                    pack = transfer(pack,source,m,case,cores)
                save_pack(run/'cache'/name,pack)
        atomic_json(run/'manifest.json',manifest)
        for case in cases:
