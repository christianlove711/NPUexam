...
第七版从 `npu_6` 独立复制，保留第六版的三来源种子重评、大图粒度细化、A/B 独立搜索、B 前四名候选池和 L2 配对评估。`--v6-run` 和 `--v7-run` 可将已验证的历史方案作为种子，仍以当前官方模拟器重新评分。官方 `code/` 未改动。
...
本项目当前把 2、3、4、5 核在 A、B、L2 三种场景下的平均加速比 **5.0** 记为挑战目标。100 例全量成绩、第四版配对对比及验证材料见 [100 例全量验证报告](100例全量验证报告_20260924.md)；目标差距与物理约束见 [性能目标与可达性](性能目标与可达性_20260924.md)。
