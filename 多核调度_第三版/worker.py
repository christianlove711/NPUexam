...
from solver_v2 import (ATTACHMENT, _graph_view, generate_candidates,
                       legacy_plan)
from solver_idblocks import generate_idblock_candidates
    ...
    if scene in ("A", "B"):
        ...
        try:
            ...
            errors.append({"label": "v2_generation",
                           "error": f"{type(exc).__name__}: {exc}"})
        try:
            candidates += [(f"v3_{label}", plan) for label, plan
                           in generate_idblock_candidates(graph, cores, scene)]
        except Exception as exc:
            errors.append({"label": "v3_generation",
                           "error": f"{type(exc).__name__}: {exc}"})
    else:
        ...
        try:
            candidates += [(f"v3_{label}", plan) for label, plan
                           in generate_idblock_candidates(graph, cores, "L2")]
        except Exception as exc:
            errors.append({"label": "v3_l2_generation",
                           "error": f"{type(exc).__name__}: {exc}"})
